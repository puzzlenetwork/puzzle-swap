...
const ctx = React.createContext<UltrastakeVM | null>(null);

export const UltrastakeVMProvider: React.FC<IProps> = ({ children }) => {
  const rootStore = useStores();
  const store = useMemo(() => new UltrastakeVM(rootStore), [rootStore]);
  return <ctx.Provider value={store}>{children}</ctx.Provider>;
};

export const useUltrastakeVM = () => useVM(ctx);
