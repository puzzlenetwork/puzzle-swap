...
const ctx = React.createContext<RangesListVm | null>(null);

export const RangesListProvider: React.FC<IProps> = ({ children }) => {
  const rootStore = useStores();
  const store = useMemo(() => new RangesListVm(rootStore), [rootStore]);
  return <ctx.Provider value={store}>{children}</ctx.Provider>;
};

export const useRangesListVm = () => useVM(ctx);
