...
import { PoolSwapVMProvider, usePoolSwapVM } from "./PoolSwapVM";
...
const PoolSwapImpl: React.FC = observer(() => {
...
const PoolSwap: React.FC = () => {
  const { poolDomain } = useParams<{ poolDomain: string }>();
  return (
    <PoolSwapVMProvider poolDomain={poolDomain ?? ""}>
      <PoolSwapImpl />
    </PoolSwapVMProvider>
  );
};
